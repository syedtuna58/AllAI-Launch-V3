...
## Recent Changes
- **November 15, 2025**: Implemented customer-centric workflow for contractors. Added Customers page showing landlords (org admins) with property and active job counts. Customers are automatically populated from organizations where the contractor has assigned work (via `contractorOrgLinks` or `assignedContractorId` on work orders). Job Hub filters now show customer filter instead of property/entity filters for contractors. Work order cards display customer names. The `/api/contractor/customers` endpoint uses batched queries to avoid N+1 issues and efficiently aggregates metrics.
